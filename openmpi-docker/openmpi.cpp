...
#include <stack>
#include <iomanip>
#include <chrono>   // Biblioteca para manipulação de formato de saída
#include <fstream> // Biblioteca para medição de tempo
#include <thread>  // Biblioteca para manipulação de threads


// Função para obter o uso de memória pelo processo atual
size_t obter_uso_da_memoria() {
    std::ifstream file("/proc/self/status"); // Abre o arquivo com informações sobre o processo
    std::string line;
    // Itera pelas linhas do arquivo
    while (std::getline(file, line)) {
    // Procura pela linha que contém "VmRSS", que indica o uso atual de memória residente
        if (line.find("VmRSS:") != std::string::npos) { // Memória que será usada pelo processo
        // Cria um stream a partir da linha
            std::istringstream iss(line); 
            std::string label;
            size_t memoria_kb;
            // Lê a linha no formato: "VmRSS: <valor> kB" e armazena o valor em KB
            iss >> label >> memoria_kb; 
            return memoria_kb;          // Retorna o uso de memória em kilobytes
        }
    }
    return 0; // Retorna 0 caso não consiga encontrar ou ler a informação
}
// Função para obter o tempo de uso de CPU ocupado e o tempo total
std::pair<double, double> obter_tempo_cpu() {
    std::ifstream file("/proc/stat"); // Abre o arquivo com estatísticas do sistema
    std::string line;
...
// Alias para o tipo ordered_json, garantindo que as chaves do JSON sejam mantidas ordenadas
using ordered_json = nlohmann::ordered_json;

// Representação do grafo como um mapa não ordenado (hash table)
// onde cada nó mapeia para um vetor de nós adjacentes
...
void load_graph(const std::string &filename) {
    ...
    if (rank == 0) {
        ...
        while (std::getline(file, line)) {
            ...
            graph[to]; // Garante que os nós isolados também sejam representados no grafo
            total_edges++; // Incrementa o número total de arestas
        }
        total_nodes = graph.size(); // Define o número total de nós no grafo
        file.close();
    }

    // Realiza o broadcast do número total de nós e arestas para todos os processos
...
// Função para calcular o maior componente fortemente conectado (WCC)
std::pair<int, int> largest_wcc() {
    ...
    auto bfs_bidirectional = [](int start_node, const std::unordered_map<int, std::vector<int>>& graph,
        ...
        int nodes_count = 0;      // Contador para o número de nós no componente
        int edges_count = 0;     // Contador para o número de arestas no componente
        ...
        while (!q.empty()) {
            ...
            // Processa todos os vizinhos do nó atual
        ...
        // Retorna o número de nós e arestas no componente
        return std::make_pair(nodes_count, edges_count);
    };

    // Constrói uma representação bidirecional do grafo
    ...
    // Itera por todos os nós do grafo bidirecional
...
double average_clustering_coefficient() {
    ...
    for (const auto& [node, neighbors] : graph) {
        ...
        // Exibir progresso no terminal a cada 1.000 nós processados
        if (valid_nodes % 10000 == 0) {
            std::cout << "Nós " << valid_nodes << " processados para coeficiente de cluster..." << std::endl;
        }
    }
...
// Fração de triângulos fechados em relação às combinações possíveis
std::pair<int, double> calculate_triangles() {
    ...
    for (const auto& [node, neighbors] : bidirectional_graph) {
        ...
        if (processed_nodes % 1000 == 0) {
            std::cout << "Nós " << processed_nodes << " processados para triângulos..." << std::endl;
        }
    }
...
std::pair<int, double> calculate_diameter() {
    ...
    std::cout << "Subgrafo WCC criado com" << wcc_subgraph.size() << " nós." << std::endl;
    ...
    for (const auto& [node, _] : wcc_subgraph) {
        ...
        processed++;
        if (processed % 10 == 0) {
            std::cout << "Nós processados " << processed << " para aproximação de diâmetro..." << std::endl;
        }
    }
    ...
    std::cout << "Cálculo do diâmetro concluído." << std::endl;
    std::cout << "Diâmetro máximo global: " << global_max_diameter << std::endl;
    std::cout << "Diâmetro Efetivo (90%): " << effective_diameter << std::endl;
...
int main(int argc, char** argv) {
    ...
    size_t initial_memory = obter_uso_da_memoria();
    auto [start_cpu_busy, start_cpu_total] = obter_tempo_cpu();
    ...
    size_t final_memory = obter_uso_da_memoria();
    auto [end_cpu_busy, end_cpu_total] = obter_tempo_cpu();
    ...
    std::cout << "\nTemp de execução - wall-clock: " << elapsed_time << " segundos\n";
    std::cout << "Temp de CPU ocupado: " << cpu_busy_time << " segundos\n";
    std::cout << "Temp de CPU ocioso: " << cpu_idle_time << " segundos\n";
    std::cout << "Consu de memoria - inicial: " << initial_memory << " KB\n";
    std::cout << "Consu de memoria - final: " << final_memory << " KB\n";

      // Registro das métricas em arquivo
    std::ofstream arquivo_saida("../metricas_openmpi.txt", std::ios::app);
    if (arquivo_saida.is_open()) {
        arquivo_saida << "======================\n";
        arquivo_saida << "Métricas de uso do OpenMPI\n";
        arquivo_saida << "-----------------------\n";
        arquivo_saida << "Tempo total (wall-clock): " << elapsed_time << " segundos\n";
        arquivo_saida << "-----------------------\n";
        arquivo_saida << "Tempo de CPU ocupado: " << cpu_busy_time << " segundos\n";
        arquivo_saida << "-----------------------\n";
        arquivo_saida << "Tempo de CPU inativo: " << cpu_idle_time << " segundos\n";
        arquivo_saida << "-----------------------\n";
        arquivo_saida << "Consumo de memória (inicial): " << initial_memory << " KB\n";
        arquivo_saida << "-----------------------\n";
        arquivo_saida << "Consumo de memória (final): " << final_memory << " KB\n";
        arquivo_saida << "-----------------------\n";
        arquivo_saida.close();
        std::cout << "Métricas gravadas em 'metricas_openmpi.txt'.\n";
    } else {
        std::cerr << "Falha ao abrir o arquivo para registrar as métricas.\n";
    }
        return 0;
